...
use iso2god::{game_list, god, iso_fs};
...
fn main() -> Result<(), Error> {
    ...
    let executable = source_iso
        .root_dir()
        .read_entries(&mut source_iso)?
        .map(|e| e.expect("error reading root dir"))
        .find(|e| {
            e.name().eq_ignore_ascii_case(b"default.xbe")
                || e.name().eq_ignore_ascii_case(b"default.xex")
        })
        .expect("default executable not found");

    let executable_reader = source_iso.seek_to_sector(executable.data_extent.sector)?;
    let title_info = match executable.name() {
        b"default.xex" => TitleInfo::from_xex(executable_reader)?,
        b"default.xbe" => TitleInfo::from_xbe(executable_reader)?,
        _ => unreachable!(),
    };
