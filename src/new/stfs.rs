...
impl<R: Read + Seek> ReadFromSlice<R> for Package {
    ...
    fn read_from_slice(rs: &mut ReadSlice<R>) -> Result<Self, Self::Error> {
        ...
        let r = rs.by_ref().seek_to_offset(0x0344)?;
        ...
        let file_system_descriptor_range = 0x244..(0x244 + 0x24);
        ...
        let file_system = match file_system_type {
            0 => FileSystem::read_stfs(rs.by_ref().slice(file_system_descriptor_range))?,
            1 => FileSystem::read_svod(rs.by_ref().slice(file_system_descriptor_range))?,
            _ => {
        ...
        if metadata_version >= 1 {
            let r = rs.by_ref().seek_to_offset(0x03b1)?;
        ...
        let mut r = rs.by_ref().seek_to_offset(0x03fd)?;
        ...
        let r = rs.by_ref().seek_to_offset(0x1712)?;
        ...
        if metadata_version >= 1 {
            display_name.read_additional_from(rs.by_ref().seek_to_offset(0x541a)?)?;
            display_description.read_additional_from(rs.by_ref().seek_to_offset(0x941a)?)?;
        }
...
impl FileSystem {
    fn read_stfs<R: Read + Seek>(_rs: ReadSlice<R>) -> Result<Self, std::io::Error> {
        Ok(Self::Stfs {})
    }

    fn read_svod<R: Read + Seek>(rs: ReadSlice<R>) -> Result<Self, std::io::Error> {
        use std::io::{Error, ErrorKind::*};

        let mut r = rs.seek_to_start()?;
...
impl LocalizedString {
    pub fn read_from(mut r: impl Read) -> Result<Self, std::io::Error> {
    ...
    pub fn read_additional_from(&mut self, mut r: impl Read) -> Result<(), std::io::Error> {
        self.0.reserve(6);
        for _ in 0..6 {
            let s = SmallString::read_from(&mut r)?;
            self.0.push(s);
...
impl SmallString {
    pub fn read_from(r: impl Read) -> Result<Self, std::io::Error> {
        let mut buf = [0u16; 64];
        r.chain(std::io::repeat(0)).read_u16_into::<BE>(&mut buf)?;
